...
<div class="card">
  ...
  <p class="err-desc">Our servers hit a bump. This has been logged and our team is on it. Please try again shortly.</p>
  <div class="actions">
    <a href="${pageContext.request.contextPath}/" class="btn btn-grad">✈ Back to Home</a>
      <a href="javascript:location.reload()" class="btn btn-ghost">↺ Retry</a>
  </div>
  <% if (exception != null) { %><div style='margin-top:20px;font-size:.74rem;color:var(--muted);background:var(--bg);padding:8px 14px;border-radius:8px;text-align:left;font-family:monospace;word-break:break-all'><%= exception.getClass().getSimpleName() %>: <%= exception.getMessage() != null ? exception.getMessage().substring(0,Math.min(80,exception.getMessage().length())) : "" %>…</div><% } %>
</div>
<script>
