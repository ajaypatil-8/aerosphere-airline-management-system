...
<div class="as-main">
  ...
  <div class="page-header animate-fadeup">
    <div class="page-title"><i class="ph-bold ph-user"></i> Users Report</div>
    <div class="page-subtitle">Generated: <%=gen%> &nbsp;·&nbsp; <%=users!=null?users.size():0%> records</div>
  </div>
  <div class="table-wrap animate-fadeup">
    <% if(users==null||users.isEmpty()){ %>
      <div class="empty-state"><div class="empty-icon"><i class="ph-bold ph-user"></i></div><h3>No data available</h3><p style="font-size:.83rem;margin-top:8px">The report servlet may not have returned data. Check your database connection and that the servlet is properly mapping the request attribute.</p></div>
    <%}else{%>
...
<div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;padding:16px 0 4px">
  ...
  <div style="display:flex;gap:4px;flex-wrap:wrap">
    <%if(_cp>1){%><a href="?role=<%=role!=null?java.net.URLEncoder.encode(role,"UTF-8"):""%>&amp;search=<%=search!=null?java.net.URLEncoder.encode(search,"UTF-8"):""%>&amp;page=1" style="<%=_ps()%>">«</a><%}%>
    <%if(_cp>1){%><a href="?role=<%=role!=null?java.net.URLEncoder.encode(role,"UTF-8"):""%>&amp;search=<%=search!=null?java.net.URLEncoder.encode(search,"UTF-8"):""%>&amp;page=<%=_cp-1%>" style="<%=_ps()%>">‹</a><%}%>
    <%for(int _pg=_s;_pg<=_e;_pg++){%><a href="?role=<%=role!=null?java.net.URLEncoder.encode(role,"UTF-8"):""%>&amp;search=<%=search!=null?java.net.URLEncoder.encode(search,"UTF-8"):""%>&amp;page=<%=_pg%>" style="<%=_ps(_pg==_cp)%>"><%=_pg%></a><%}%>
    <%if(_cp<_tp){%><a href="?role=<%=role!=null?java.net.URLEncoder.encode(role,"UTF-8"):""%>&amp;search=<%=search!=null?java.net.URLEncoder.encode(search,"UTF-8"):""%>&amp;page=<%=_cp+1%>" style="<%=_ps()%>">›</a><%}%>
    <%if(_cp<_tp){%><a href="?role=<%=role!=null?java.net.URLEncoder.encode(role,"UTF-8"):""%>&amp;search=<%=search!=null?java.net.URLEncoder.encode(search,"UTF-8"):""%>&amp;page=<%=_tp%>" style="<%=_ps()%>">»</a><%}%>
  </div>
</div>
<%}}%>
    <%}%>
