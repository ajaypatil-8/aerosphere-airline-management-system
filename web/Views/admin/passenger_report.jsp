...
<div class="as-main">
  ...
  <div class="page-header animate-fadeup">
    <div class="page-title"><i class="ph-bold ph-suitcase"></i> Passengers Report</div>
  ...
  <div class="table-wrap animate-fadeup">
    <% if(passengers==null||passengers.isEmpty()){ %>
      <div class="empty-state"><div class="empty-icon"><i class="ph-bold ph-suitcase"></i></div><h3>No data available</h3><p style="font-size:.83rem;margin-top:8px">The report servlet may not have returned data. Check your database connection and that the servlet is properly mapping the request attribute.</p></div>
    <%}else{%>
    <table class="sc-table">
      <thead><tr><th>#</th><th>Passenger</th><th>Flight</th><th>Route</th><th>Seat</th><th>Age</th><th>Gender</th><th>Status</th></tr></thead>
      <tbody>
        <% int i=0; for(PassengerRow p:passengers){i++;
...
<tr>
  ...
  <td style="color:var(--sky);font-weight:800"><%=p.flightNo%></td>
  <td style="font-size:.83rem"><%=p.source%> <span style="color:var(--sky)">→</span> <%=p.destination%></td>
  <td style="color:var(--emerald);font-weight:700"><%=p.seatNo!=null?p.seatNo:"—"%></td>
  <td><%=p.age%></td>
  <td><%=p.gender%></td>
...
<div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;padding:16px 0 4px">
  ...
  <div style="display:flex;gap:4px;flex-wrap:wrap">
    <%if(_cp>1){%><a href="?flightId=<%=flightId!=null?java.net.URLEncoder.encode(flightId,"UTF-8"):""%>&amp;page=1" style="<%=_ps()%>">«</a><%}%>
    <%if(_cp>1){%><a href="?flightId=<%=flightId!=null?java.net.URLEncoder.encode(flightId,"UTF-8"):""%>&amp;page=<%=_cp-1%>" style="<%=_ps()%>">‹</a><%}%>
    <%for(int _pg=_s;_pg<=_e;_pg++){%><a href="?flightId=<%=flightId!=null?java.net.URLEncoder.encode(flightId,"UTF-8"):""%>&amp;page=<%=_pg%>" style="<%=_ps(_pg==_cp)%>"><%=_pg%></a><%}%>
    <%if(_cp<_tp){%><a href="?flightId=<%=flightId!=null?java.net.URLEncoder.encode(flightId,"UTF-8"):""%>&amp;page=<%=_cp+1%>" style="<%=_ps()%>">›</a><%}%>
    <%if(_cp<_tp){%><a href="?flightId=<%=flightId!=null?java.net.URLEncoder.encode(flightId,"UTF-8"):""%>&amp;page=<%=_tp%>" style="<%=_ps()%>">»</a><%}%>
  </div>
</div>
<%}}%>
    <%}%>
