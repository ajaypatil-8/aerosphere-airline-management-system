...
<div class="as-main">
  ...
  <div class="page-header animate-fadeup">
    <div class="page-title"><i class="ph-bold ph-airplane-tilt"></i> Flights Report</div>
    <div class="page-subtitle">Generated: <%=gen%> &nbsp;·&nbsp; <%=flights!=null?flights.size():0%> records</div>
  </div>
  <div class="table-wrap animate-fadeup">
    <% if(flights==null||flights.isEmpty()){ %>
      <div class="empty-state"><div class="empty-icon"><i class="ph-bold ph-airplane-tilt"></i></div><h3>No data available</h3><p style="font-size:.83rem;margin-top:8px">The report servlet may not have returned data. Check your database connection and that the servlet is properly mapping the request attribute.</p></div>
    <%}else{%>
    <table class="sc-table">
      ...
      <tbody>
        <% int i=0; for(FlightRow f:flights){i++;
  int pct=f.totalSeats>0?(int)((f.totalSeats-f.availableSeats)*100.0/f.totalSeats):0;
  String sc=pct<50?"var(--emerald)":pct<80?"#B8863F":"#B3554A"; %>
<tr>
  <td style="color:var(--text-muted)"><%=i%></td>
  <td style="color:var(--sky);font-weight:800"><%=f.flightNo%></td>
  <td style="font-size:.83rem"><%=f.source%> <span style="color:var(--sky)">→</span> <%=f.destination%></td>
  ...
  <td style="color:var(--emerald);font-weight:700">₹<%=String.format("%,.0f",f.price)%></td>
  <td><%=f.totalSeats%></td>
  <td><%=f.availableSeats%></td>
  <td style="min-width:80px"><div style="height:7px;background:var(--border);border-radius:99px;overflow:hidden"><div style="height:100%;width:<%=pct%>%;background:<%=sc%>;border-radius:99px;transition:width .4s"></div></div><span style="font-size:.72rem;color:<%=sc%>"><%=pct%>%</span></td>
</tr>
<%}%>
      </tbody>
    </table>
...
<div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;padding:16px 0 4px">
  ...
  <div style="display:flex;gap:4px;flex-wrap:wrap">
    <%if(_cp>1){%><a href="?route=<%=route!=null?java.net.URLEncoder.encode(route,"UTF-8"):""%>&amp;date=<%=date!=null?java.net.URLEncoder.encode(date,"UTF-8"):""%>&amp;dateFrom=<%=dateFrom!=null?java.net.URLEncoder.encode(dateFrom,"UTF-8"):""%>&amp;dateTo=<%=dateTo!=null?java.net.URLEncoder.encode(dateTo,"UTF-8"):""%>&amp;page=1" style="<%=_ps()%>">«</a><%}%>
    <%if(_cp>1){%><a href="?route=<%=route!=null?java.net.URLEncoder.encode(route,"UTF-8"):""%>&amp;date=<%=date!=null?java.net.URLEncoder.encode(date,"UTF-8"):""%>&amp;dateFrom=<%=dateFrom!=null?java.net.URLEncoder.encode(dateFrom,"UTF-8"):""%>&amp;dateTo=<%=dateTo!=null?java.net.URLEncoder.encode(dateTo,"UTF-8"):""%>&amp;page=<%=_cp-1%>" style="<%=_ps()%>">‹</a><%}%>
    <%for(int _pg=_s;_pg<=_e;_pg++){%><a href="?route=<%=route!=null?java.net.URLEncoder.encode(route,"UTF-8"):""%>&amp;date=<%=date!=null?java.net.URLEncoder.encode(date,"UTF-8"):""%>&amp;dateFrom=<%=dateFrom!=null?java.net.URLEncoder.encode(dateFrom,"UTF-8"):""%>&amp;dateTo=<%=dateTo!=null?java.net.URLEncoder.encode(dateTo,"UTF-8"):""%>&amp;page=<%=_pg%>" style="<%=_ps(_pg==_cp)%>"><%=_pg%></a><%}%>
    <%if(_cp<_tp){%><a href="?route=<%=route!=null?java.net.URLEncoder.encode(route,"UTF-8"):""%>&amp;date=<%=date!=null?java.net.URLEncoder.encode(date,"UTF-8"):""%>&amp;dateFrom=<%=dateFrom!=null?java.net.URLEncoder.encode(dateFrom,"UTF-8"):""%>&amp;dateTo=<%=dateTo!=null?java.net.URLEncoder.encode(dateTo,"UTF-8"):""%>&amp;page=<%=_cp+1%>" style="<%=_ps()%>">›</a><%}%>
    <%if(_cp<_tp){%><a href="?route=<%=route!=null?java.net.URLEncoder.encode(route,"UTF-8"):""%>&amp;date=<%=date!=null?java.net.URLEncoder.encode(date,"UTF-8"):""%>&amp;dateFrom=<%=dateFrom!=null?java.net.URLEncoder.encode(dateFrom,"UTF-8"):""%>&amp;dateTo=<%=dateTo!=null?java.net.URLEncoder.encode(dateTo,"UTF-8"):""%>&amp;page=<%=_tp%>" style="<%=_ps()%>">»</a><%}%>
  </div>
</div>
<%}}%>
    <%}%>
