...
<div class="as-main">
  ...
  <div class="page-header animate-fadeup">
    <div class="page-title"><i class="ph-bold ph-x-circle"></i> Cancellations Report</div>
  ...
  <div class="table-wrap animate-fadeup">
    <% if(cancelled==null||cancelled.isEmpty()){ %>
      <div class="empty-state"><div class="empty-icon"><i class="ph-bold ph-x-circle"></i></div><h3>No data available</h3><p style="font-size:.83rem;margin-top:8px">The report servlet may not have returned data. Check your database connection and that the servlet is properly mapping the request attribute.</p></div>
    <%}else{%>
    <table class="sc-table">
      <thead><tr><th>#</th><th>Booking ID</th><th>Passenger</th><th>Flight</th><th>Route</th><th>Seats</th><th>Amount</th><th>Payment Status</th><th>Booked On</th></tr></thead>
      <tbody>
...
<tr>
  ...
  <td style="color:var(--sky);font-weight:700">#<%=c.bookingId%></td>
  <td style="font-weight:600"><%=c.userName%></td>
  <td style="font-weight:700;color:var(--emerald)"><%=c.flightNo%></td>
  <td style="font-size:.83rem"><%=c.route%></td>
  <td><%=c.seats%></td>
  <td style="color:var(--emerald);font-weight:700">₹<%=String.format("%,.0f",c.amount)%></td>
