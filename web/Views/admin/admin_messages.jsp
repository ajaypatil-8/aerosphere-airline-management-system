...
.page-title{font-family:'Fraunces',sans-serif;font-size:1.4rem;font-weight:800}
.msg-filters{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:20px}
.filter-btn{padding:7px 16px;border-radius:var(--radius-sm);border:1.5px solid var(--border);font-size:.83rem;font-weight:600;cursor:pointer;background:var(--surface-0);color:var(--text-muted);transition:all .2s;font-family:'Inter',sans-serif}
...
.badge-replied{background:rgba(91,138,110,.12);color:#3E6350;font-size:.65rem;font-weight:700;padding:2px 7px;border-radius:99px;border:1px solid rgba(91,138,110,.3)}
...
.reply-form textarea{width:100%;padding:12px 14px;background:var(--bg);border:1.5px solid var(--border-2);border-radius:var(--radius-sm);color:var(--text);font-family:'Inter',sans-serif;font-size:.88rem;outline:none;resize:vertical;min-height:110px;transition:border-color .2s,box-shadow .2s}
...
.btn-reply{padding:9px 20px;background:var(--grad-brand);color:#fff;border:none;border-radius:var(--radius-sm);font-weight:700;font-size:.85rem;cursor:pointer;font-family:'Inter',sans-serif;transition:opacity .2s}
.btn-reply:hover{opacity:.88}
.btn-delete{padding:9px 16px;background:transparent;color:var(--danger);border:1.5px solid var(--danger);border-radius:var(--radius-sm);font-weight:600;font-size:.83rem;cursor:pointer;font-family:'Inter',sans-serif;transition:all .2s}
.btn-delete:hover{background:var(--danger);color:#fff}
.replied-badge-row{display:inline-flex;align-items:center;gap:6px;padding:8px 14px;background:rgba(91,138,110,.08);border:1px solid rgba(91,138,110,.25);border-radius:var(--radius-sm);font-size:.82rem;color:#3E6350;font-weight:600;margin-top:12px}
.empty-state{text-align:center;padding:56px 20px;color:var(--text-muted)}
.empty-state .icon{font-size:3rem;margin-bottom:12px;opacity:.4}
.alert-success{background:rgba(91,138,110,.08);border:1px solid rgba(91,138,110,.3);border-radius:var(--radius);padding:12px 18px;color:#3E6350;font-size:.88rem;margin-bottom:16px}
.alert-error{background:var(--danger-bg);border:1px solid var(--danger-border);border-radius:var(--radius);padding:12px 18px;color:var(--danger-dark);font-size:.88rem;margin-bottom:16px}
...
.stat-mini-num{font-family:'Fraunces',sans-serif;font-size:1.6rem;font-weight:800;color:var(--primary)}
...
<div class="as-main">
  <div class="topbar">
    <button class="as-sidebar-toggle" onclick="document.getElementById('asSidebar').classList.toggle('open')"><i class="ph-bold ph-list"></i></button>
    <span class="topbar-title"><i class="ph-bold ph-envelope-simple"></i> Contact Messages</span>
    <div style="margin-left:auto;display:flex;gap:8px">
      <button class="theme-toggle" onclick="AS.toggleTheme()" style="width:34px;height:34px;border:1px solid var(--border);border-radius:8px;background:var(--surface-0);cursor:pointer;font-size:.9rem"><i class="ph-bold ph-moon"></i></button>
    </div>
  </div>

  <div class="page-wrap">
    <% if (success != null) { %><div class="alert-success"><i class="ph-bold ph-check-circle"></i> <%= success %></div><% } %>
    <% if (error != null) { %><div class="alert-error"><i class="ph-bold ph-warning"></i> <%= error %></div><% } %>

    <div class="page-header">
      <div class="page-title"><i class="ph-bold ph-tray"></i> User Messages</div>
    </div>
    ...
    <div class="stat-row">
      ...
      <div class="stat-mini"><div class="stat-mini-num" style="color:#B8863F"><%= unread %></div><div class="stat-mini-label">Unread</div></div>
      <div class="stat-mini"><div class="stat-mini-num" style="color:#5B8A6E"><%= replied %></div><div class="stat-mini-label">Replied</div></div>
    </div>

    <div class="msg-filters">
      <button class="filter-btn active" onclick="filterMsgs('all',this)">All (<%= total %>)</button>
      <button class="filter-btn" onclick="filterMsgs('NEW',this)"><i class="ph-bold ph-circle"></i> Unread (<%= unread %>)</button>
      <button class="filter-btn" onclick="filterMsgs('REPLIED',this)"><i class="ph-bold ph-check-circle"></i> Replied (<%= replied %>)</button>
    </div>

    <% if (messages == null || messages.isEmpty()) { %>
      <div class="empty-state">
        <div class="icon"><i class="ph-bold ph-tray"></i></div>
        <h3 style="font-family:'Fraunces',sans-serif;font-size:1.1rem;font-weight:700;color:var(--text);margin-bottom:8px">No messages yet</h3>
      ...
      <div class="msg-card <%= isNew ? "unread" : "" %>" data-status="<%= status %>">
        <div class="msg-header" onclick="toggleMsg(this.parentElement)">
          <div class="msg-meta">
            ...
            <div class="msg-subject"><i class="ph-bold ph-push-pin"></i> <%= msg.get("subject") %></div>
        ...
        <div class="msg-body">
          <div class="msg-body-inner">
            ...
            <% if ("REPLIED".equals(status) && msg.get("admin_reply") != null) { %>
              <div style="font-size:.78rem;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:#3E6350;margin-top:16px;margin-bottom:6px">Your Reply (sent)</div>
              <div class="msg-content" style="border-left:3px solid #5B8A6E"><%= msg.get("admin_reply") %></div>
            <% } %>
            ...
            <form class="reply-form" action="${pageContext.request.contextPath}/adminMessages" method="post">
              ...
              <div class="reply-actions">
                <button type="submit" class="btn-reply"><i class="ph-bold ph-paper-plane-tilt"></i> Send Reply</button>
                <form action="${pageContext.request.contextPath}/adminMessages" method="post" style="display:inline">
                  <input type="hidden" name="action" value="delete">
                  <input type="hidden" name="messageId" value="<%= msg.get("id") %>">
                  <input type="hidden" name="_csrf" value="<%= _csrfToken %>">
                  <button type="submit" class="btn-delete" onclick="return confirm('Delete this message?')"><i class="ph-bold ph-trash"></i> Delete</button>
                </form>
              </div>
            </form>
            <% } else { %>
            <div class="reply-actions">
              <div class="replied-badge-row"><i class="ph-bold ph-check-circle"></i> Reply sent to <%= msg.get("sender_email") %></div>
