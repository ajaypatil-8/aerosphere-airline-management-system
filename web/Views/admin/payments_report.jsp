...
<title>Payments Report – AeroSphere</title>
...
<div class="page-wrapper">
  ...
  <div class="table-wrap animate-fadeup">
    ...
    <td style="color:var(--primary)">#<%= p.paymentId %></td>
    <td>#<%= p.bookingId %></td><td><%= p.userName %></td>
    <td><strong><%= p.flightNo %></strong></td>
    <td style="font-size:.84rem"><%= p.route %></td>
    <td style="color:var(--primary);font-weight:700">₹<%= String.format("%,.0f",p.amount) %></td>
    ...
    <td style="font-size:.8rem;color:var(--text-muted)"><%= p.paymentDate!=null?p.paymentDate.toString().substring(0,10):"—" %></td></tr>
    <% } %></tbody></table>
    <% } %>
  </div>
</div>
