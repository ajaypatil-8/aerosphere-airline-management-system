...
<title>Revenue Report – AeroSphere</title>
...
<div class="as-main">
  ...
  <div class="page-header animate-fadeup">
    <div class="page-title"><i class="ph-bold ph-credit-card"></i> Revenue Report</div>
    <div class="page-subtitle">Generated: <%=gen%> &nbsp;·&nbsp; <%=payments!=null?payments.size():0%> records</div>
  </div>
  <div class="table-wrap animate-fadeup">
    <% if(payments==null||payments.isEmpty()){ %>
      <div class="empty-state"><div class="empty-icon"><i class="ph-bold ph-credit-card"></i></div><h3>No data available</h3><p style="font-size:.83rem;margin-top:8px">The report servlet may not have returned data. Check your database connection and that the servlet is properly mapping the request attribute.</p></div>
    <%}else{%>
    <table class="sc-table">
      <thead><tr><th>#</th><th>Pay ID</th><th>Booking</th><th>Passenger</th><th>Flight</th><th>Route</th><th>Amount</th><th>Method</th><th>Status</th><th>Date</th></tr></thead>
      <tbody>
        <% int i=0; for(PaymentRow p:payments){i++;
...
<tr>
  ...
  <td style="color:var(--sky);font-weight:700">#<%=p.paymentId%></td>
  ...
  <td style="font-size:.8rem;color:var(--text-muted)"><%=p.paymentDate!=null?p.paymentDate.toString().substring(0,10):"—"%></td>
</tr>
<%}%>
      </tbody>
    </table>
...
<div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;padding:16px 0 4px">
  ...
  <div style="display:flex;gap:4px;flex-wrap:wrap">
    <%if(_cp>1){%><a href="?dateFrom=<%=dateFrom!=null?java.net.URLEncoder.encode(dateFrom,"UTF-8"):""%>&amp;dateTo=<%=dateTo!=null?java.net.URLEncoder.encode(dateTo,"UTF-8"):""%>&amp;method=<%=method!=null?java.net.URLEncoder.encode(method,"UTF-8"):""%>&amp;page=1" style="<%=_ps()%>">«</a><%}%>
    <%if(_cp>1){%><a href="?dateFrom=<%=dateFrom!=null?java.net.URLEncoder.encode(dateFrom,"UTF-8"):""%>&amp;dateTo=<%=dateTo!=null?java.net.URLEncoder.encode(dateTo,"UTF-8"):""%>&amp;method=<%=method!=null?java.net.URLEncoder.encode(method,"UTF-8"):""%>&amp;page=<%=_cp-1%>" style="<%=_ps()%>">‹</a><%}%>
    <%for(int _pg=_s;_pg<=_e;_pg++){%><a href="?dateFrom=<%=dateFrom!=null?java.net.URLEncoder.encode(dateFrom,"UTF-8"):""%>&amp;dateTo=<%=dateTo!=null?java.net.URLEncoder.encode(dateTo,"UTF-8"):""%>&amp;method=<%=method!=null?java.net.URLEncoder.encode(method,"UTF-8"):""%>&amp;page=<%=_pg%>" style="<%=_ps(_pg==_cp)%>"><%=_pg%></a><%}%>
    <%if(_cp<_tp){%><a href="?dateFrom=<%=dateFrom!=null?java.net.URLEncoder.encode(dateFrom,"UTF-8"):""%>&amp;dateTo=<%=dateTo!=null?java.net.URLEncoder.encode(dateTo,"UTF-8"):""%>&amp;method=<%=method!=null?java.net.URLEncoder.encode(method,"UTF-8"):""%>&amp;page=<%=_cp+1%>" style="<%=_ps()%>">›</a><%}%>
    <%if(_cp<_tp){%><a href="?dateFrom=<%=dateFrom!=null?java.net.URLEncoder.encode(dateFrom,"UTF-8"):""%>&amp;dateTo=<%=dateTo!=null?java.net.URLEncoder.encode(dateTo,"UTF-8"):""%>&amp;method=<%=method!=null?java.net.URLEncoder.encode(method,"UTF-8"):""%>&amp;page=<%=_tp%>" style="<%=_ps()%>">»</a><%}%>
  </div>
</div>
<%}}%>
    <%}%>
