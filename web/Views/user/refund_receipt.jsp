...
.receipt-top::before{content:'';position:absolute;top:0;left:0;right:0;height:4px;background:<%=isApproved?"var(--grad)":isRejected?"linear-gradient(135deg,#B3554A,#96453B)":"linear-gradient(135deg,#B8863F,#8A6530)"%>}
.status-icon{font-size:3rem;margin-bottom:12px;display:block}
.status-title{font-family:'Fraunces',serif;font-size:1.4rem;font-weight:800;letter-spacing:-.5px;margin-bottom:6px;color:<%=isApproved?"var(--emerald)":isRejected?"#B3554A":"#8A6530"%>}
...
.amount-value{background:var(--grad);-webkit-background-clip:text;-webkit-text-fill-color:transparent;font-family:'Fraunces',serif;font-size:1.15rem;font-weight:800}
.receipt-actions{padding:20px;display:flex;gap:10px;justify-content:center;border-top:1px solid var(--border)}
.btn-print{padding:9px 20px;background:var(--grad);color:#fff;border:none;border-radius:9px;font-family:'Inter',sans-serif;font-size:.83rem;font-weight:700;cursor:pointer;transition:transform .15s}
...
<div class="page-wrap">
  <div class="receipt-card fu">
    <div class="receipt-top">
      <span class="status-icon"><%=isApproved?"<i class="ph-bold ph-check-circle"></i>":isRejected?"<i class="ph-bold ph-x-circle"></i>":"<i class="ph-bold ph-hourglass"></i>"%></span>
      ...
      <div class="info-box">
        <%if(flightNo!=null){%>
        <div class="info-row"><div class="info-icon-box"><i class="ph-bold ph-airplane-tilt"></i></div><div><div class="info-label">Flight</div><div class="info-value"><%=flightNo%></div></div></div>
        <%}%>
        <%if(paxName!=null){%>
        <div class="info-row"><div class="info-icon-box"><i class="ph-bold ph-user"></i></div><div><div class="info-label">Passenger</div><div class="info-value"><%=paxName%></div></div></div>
        <%}%>
        <div class="info-row"><div class="info-icon-box"><i class="ph-bold ph-coins"></i></div><div><div class="info-label">Refund Amount</div><div class="info-value amount-value">₹<%=String.format("%,.2f",amount)%></div></div></div>
        <div class="info-row"><div class="info-icon-box"><i class="ph-bold ph-clipboard-text"></i></div><div><div class="info-label">Status</div><div class="info-value" style="color:<%=isApproved?"var(--emerald)":isRejected?"#B3554A":"#8A6530"%>"><%=st%></div></div></div>
        <%if(isApproved&&approved!=null){%>
        <div class="info-row"><div class="info-icon-box"><i class="ph-bold ph-calendar-blank"></i></div><div><div class="info-label">Processed On</div><div class="info-value"><%=approved.toString().substring(0,10)%></div></div></div>
        <%}%>
      </div>
    </div>
    <div class="receipt-actions">
      <button onclick="window.print()" class="btn-print"><i class="ph-bold ph-printer"></i> Print Receipt</button>
